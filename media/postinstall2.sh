...
#   6) Extract VM from 7z (auto-discovered)
#   7) Flatten nested win10/
#   8) Locate .vmx (auto-discovered)
#   9) Patch .vmx
...
# Optional: let caller pin an exact VMX; otherwise we auto-discover
VMX_PATH_OVERRIDE=${VMX_PATH_OVERRIDE:-}

SPLIT_DIR=${SPLIT_DIR:-"${SEED_DST}/split"}
# Optional: if set, we’ll prefer this exact archive; otherwise we auto-discover
...
log "CONFIG: VM_ROOT=$VM_ROOT, VMX_PATH_DEFAULT=$VMX_PATH_DEFAULT, VMX_PATH_OVERRIDE=$VMX_PATH_OVERRIDE"
...
for i in $(seq 1 20); do
  ...
  command -v wmctrl >/devnull 2>&1 && wmctrl -F -c "Welcome to Ubuntu" 2>/dev/null || true
  sleep 1
done
...
# 1) If VM_ARCHIVE is explicitly set and exists, prefer that
if [[ -n "${VM_ARCHIVE:-}" && -f "$VM_ARCHIVE" ]]; then
  archive="$VM_ARCHIVE"
  log "Using VM_ARCHIVE from config: $archive"
else
  # 2) Otherwise, auto-discover under $SPLIT_DIR
  if [[ -d "$SPLIT_DIR" ]]; then
    # Prefer multi-part archives: *.7z.001
    first_split=$(find "$SPLIT_DIR" -maxdepth 1 -type f -name '*.7z.001' | sort | head -n1 || true)
    if [[ -n "$first_split" ]]; then
      archive="$first_split"
      log "Discovered 7z split set starting at: $archive"
    else
      # Fallback: single-file *.7z
      single_7z=$(find "$SPLIT_DIR" -maxdepth 1 -type f -name '*.7z' | sort | head -n1 || true)
      if [[ -n "$single_7z" ]]; then
        archive="$single_7z"
        log "Discovered single 7z archive: $archive"
      fi
    fi
  else
    log "[INFO] Split directory does not exist: $SPLIT_DIR"
  fi
fi

if [[ -n "$archive" ]]; then
  log "Extracting: $archive → $VM_ROOT"
  sudo -u "$USER_NAME" "$SEVEN_Z" x -aoa -o"$VM_ROOT" "$archive"
else
  log "[INFO] No VM archive (.7z or .7z.001) found under $SPLIT_DIR (skipping extraction)"
fi
...
if [[ -n "$VMX_PATH_OVERRIDE" && -f "$VMX_PATH_OVERRIDE" ]]; then
  VMX_PATH="$VMX_PATH_OVERRIDE"
  log "Using VMX from override: $VMX_PATH"
elif [[ -f "$VMX_PATH_DEFAULT" ]]; then
  VMX_PATH="$VMX_PATH_DEFAULT"
  log "Using default VMX path: $VMX_PATH"
else
  # Generic search: any *.vmx under VM_ROOT
  VMX_PATH=$(find "$VM_ROOT" -type f -name '*.vmx' | sort | head -n1 || true)
  if [[ -n "$VMX_PATH" ]]; then
    log "Discovered VMX via search: $VMX_PATH"
  fi
fi

[[ -n "$VMX_PATH" ]] || { log "[ERROR] Could not locate any .vmx under $VM_ROOT"; exit 1; }
log "Final VMX selection: $VMX_PATH"
